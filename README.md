...
- Choose what to write under **Settings → Advanced** (Cmd+,):
  ...
  - **Convert HEIC to JPEG** — off (default) preserves the original HEIC/HEIF format; on re-encodes HEIC captures as high-quality JPEG on export, useful if your destination doesn't understand HEIC. Non-HEIC photos are unaffected. **Replace already-exported HEIC files** (off by default, shown while the conversion toggle is on) makes the next export run swap older HEIC files for their JPEG exports — Live Photo paired videos are rewritten too so each pair keeps one filename; only files Photo Export itself created are removed
