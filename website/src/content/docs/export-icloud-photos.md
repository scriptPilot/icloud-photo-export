...
- **Convert HEIC to JPEG** — off (default) preserves HEIC and HEIF captures in their original format; on re-encodes HEIC captures as high-quality JPEG on export — useful if your destination (a NAS, a Windows PC, an older photo viewer) doesn't understand HEIC. Non-HEIC photos are unaffected. The optional **Replace already-exported HEIC files** toggle (off by default) makes the next export run swap older HEIC files for their JPEG exports.
