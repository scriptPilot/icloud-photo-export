...
Once you've picked a scope, decide what to write. Open **Settings → Advanced** (Cmd+, or the toolbar's Settings cog) for the Format and Organization toggles:
...
The same window's red **Danger Zone** section controls how future exports
treat files already in the destination: replace files whose asset changed in
Photos, and — after an explicit confirmation — delete files of assets no longer
in the library, with folders of deleted albums and empty leftover folders
cleaned up automatically. Both options are off by default — with both on,
exports act as a mirror of the library. See
[Danger Zone (Settings → Advanced)](/photo-export/features/#danger-zone-settings-advanced)
for the details and the deletions this involves.
