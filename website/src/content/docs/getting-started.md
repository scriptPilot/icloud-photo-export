...
- **Settings → Advanced** (Cmd+,) controls what lands on disk. **Convert HEIC to JPEG** (off by default) re-encodes HEIC captures as JPEG on export, useful for destinations that don't understand HEIC; the optional **Replace already-exported HEIC files** toggle (off by default) makes the next export run swap older HEIC files for their JPEG exports. **Include originals** (off by default) adds a `_orig` companion for any photo or video edited in Photos so you keep a copy of the original bytes. **Separate videos into a subfolder** (off by default) routes standalone videos into a `videos/` subfolder so a backup doesn't mix photos and `.MOV` files. The onboarding flow exposes the first two inline on first launch.
